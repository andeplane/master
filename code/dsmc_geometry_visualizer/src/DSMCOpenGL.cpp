...
void DSMCOpenGL::initialize(int w, int h, string window_title_, GLFWkeyfun handle_keypress, GLFWmouseposfun handle_mouse_move, bool full_screen, double camera_speed) {
...
void DSMCOpenGL::set_standard_light(float t) {
    float a = 100;
...
void DSMCOpenGL::set_window_title(string window_title_) {
  window_title = window_title_;
  glfwSetWindowTitle(window_title.c_str());
  glfwPollEvents();
}

void DSMCOpenGL::init_GL() {
    // ----- GLFW Settings -----
...
CVector DSMCOpenGL::coord_to_ray(double px, double py) {
