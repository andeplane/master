...
public:
    ...
    unsigned char *get_voxel(float *r);
    void get_index_vector_from_index(const int &index, int &i, int &j, int &k);
    int get_index_of_voxel(float *r);
    double get_time_until_collision(float *r, float *v, double dt, const int &voxel_index);

    void read_matrix(string filename, DSMC_IO *io);
};

inline double Grid::get_time_until_collision(float *r, float *v, double dt, const int &voxel_index) {
    /*
     * Strategy:
...
inline int Grid::get_index_of_voxel(float *r) {
