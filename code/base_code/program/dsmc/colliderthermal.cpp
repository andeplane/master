...
void ColliderThermal::collide(Random *rnd, float *v, float *normal_vector, float *tangent_vector_1, float *tangent_vector_2)
{
    float v_normal   = sqrt_wall_temp_over_mass*sqrt(-2*log(rnd->next_double()));
    float v_tangent1 = sqrt_wall_temp_over_mass*rnd->next_gauss();
    float v_tangent2 = sqrt_wall_temp_over_mass*rnd->next_gauss();
