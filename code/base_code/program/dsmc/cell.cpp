...
void Cell::collide_molecules(float *v0, float *v1, const float &v_rel, Random *rnd) {
    float vcmx  = 0.5*(v0[0] + v1[0]);
    float vcmy  = 0.5*(v0[1] + v1[1]);
    float vcmz  = 0.5*(v0[2] + v1[2]);

    float cos_th = 1.0 - 2.0*rnd->next_double();      // Cosine and sine of
    float sin_th = sqrt(1.0 - cos_th*cos_th);        // collision angle theta
    float phi = 2*M_PI*rnd->next_double();

    float vrelx = v_rel*cos_th;                   // Compute post-collision relative velocity
    float vrely = v_rel*sin_th*cos(phi);
    float vrelz = v_rel*sin_th*sin(phi);
...
int Cell::collide(Random *rnd) {
    ...
    float crm = vr_max;     // Current maximum relative speed
    ...
    for(int isel=0; isel<collision_pairs; isel++ ) {
        ...
        float *v0 = &system->v[3*ip0];
        float *v1 = &system->v[3*ip1];
        float v_rel = sqrt(pow(v0[0] - v1[0],2) + pow(v0[1] - v1[1],2) + pow(v0[2] - v1[2],2));
...
vector<float>& Cell::update_average_velocity() {
    vector<float> new_average_velocity(3,0);
