...
private:
    ...
    void find_position_in_reservoirs(float *r, bool find_position_in_source);
    ...
    void add_molecules_from_mpi(float *data, const int &num_new_molecules);
    void remove_molecule_from_system(const long &molecule_index);
    bool validate_number_of_cells();
    inline void find_position(float *r);
    inline void find_position_in_cell(Cell *cell, float *r);
...
public:
    int cell_index_from_position(float *r);
