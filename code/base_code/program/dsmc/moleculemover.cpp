...
void MoleculeMover::do_move(float *r, float *v, double dt) {
    r[0] += v[0]*dt;
    r[1] += v[1]*dt;
    r[2] += v[2]*dt;
}

inline int get_index_of_voxel(float *r, const double &nx_div_lx,const double &ny_div_ly,const double &nz_div_lz, const int &Nz, const int &NyNz) {
    int i =  r[0]*nx_div_lx;
    int j =  r[1]*ny_div_ly;
    int k =  r[2]*nz_div_lz;
...
//    float *r = &system->r[3*molecule_index];
//    float *v = &system->v[3*molecule_index];
...
void MoleculeMover::apply_periodic_boundary_conditions(int &molecule_index, float *r, const CVector &system_length) {
...
//    int nynx = grid->Nx*grid->Ny;
    ...
    float *r = &system->r[3*molecule_index];
    float *v = &system->v[3*molecule_index];
    if(system->steps == 94) cout << "I am in thing with mole cule " << molecule_index << endl;
    ...
    if(voxels[idx] >= voxel_type_wall) {
        ...
        while(voxels[idx] != voxel_type_boundary) {
            if(system->steps == 96) cout << "I am in loop 1 with mole cule " << molecule_index << endl;
            if(voxels[idx] == voxel_type_wall) {
                tau /= 2;
        ...
        while(voxels[idx] == voxel_type_boundary) {
            if(system->steps == 96) cout << "I am in loop  2with mole cule " << molecule_index << endl;
