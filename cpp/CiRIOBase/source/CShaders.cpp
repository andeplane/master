...
void CShaderParent::ScreenToTexture(GLuint texture) {
  COpenGLPointer->buffer2texture(texture, COpenGLPointer->width, COpenGLPointer->height, COpenGL::NOMIPMAP);
}
...
void CBlurAndToonShader::Render() {
  ...
  glVertex3f(0,0,0);
  glTexCoord2d(1,1);
  glVertex3f(COpenGLPointer->width,0,0);
  glTexCoord2d(1,0);
  glVertex3f(COpenGLPointer->width,COpenGLPointer->height,0);
  glTexCoord2d(0,0);
  glVertex3f(0,COpenGLPointer->height,0);
  glEnd();
...
void CBlurAndToonShader::Start() {
  ...
  Shader->sendUniform4f((char*)"camera_input", COpenGLPointer->camera.x,COpenGLPointer->camera.y,COpenGLPointer->camera.z, 1.0);
  if (Type!=BLUR) {
...
void CBlurAndToonShader::ScreenToTexture() {
  COpenGLPointer->buffer2texture(ScreenTexture, COpenGLPointer->width, COpenGLPointer->height, COpenGL::NOMIPMAP);
}
